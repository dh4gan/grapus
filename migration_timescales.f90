...
IF(sigma_d(i)>0.0) THEN
   tmig = c_mig*aspectratio/(omega_d(i)*massratio)
ELSE
   tmig = 1.0e30*yr
ENDIF
...
ELSE
   ...
   IF(alpha_d(i)*omega_d(i)*aspectratio >=0.0) THEN
      tmig = 2.0/(3.0*alpha_d(i)*omega_d(i)*aspectratio*aspectratio)
   ELSE
      tmig = 1.0e30*yr
   ENDIF
...
if(iembryo==1) print*, 'Migration timescales: ', iembryo, migtype,  embryo(iembryo)%a/udist,embryo(iembryo)%m/mjup, &
pressure_crit, tmig/yr, tcross/yr, tgap/yr
